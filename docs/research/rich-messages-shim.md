...
`rich_message: { markdown, is_rtl, skip_entity_detection }` via `bot.do_api_request`. `RICH_ENABLED=True` by default; 404 latch (`_rich_disabled`) disables after first probe.
