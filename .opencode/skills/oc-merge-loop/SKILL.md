...
`[critical]` و `[warning]` با `REQUEST_CHANGES` یعنی must-fix پیش از مرج. `[info]` را اگر در اسکوپ و ساده (≤۵ خط) است همین حالا درست کن؛ وگرنه با دلیل و لینک به تیکت بعدی موکول کن.
