...
## ۰. گیت تازگی بیس (پیش از هر کاری)
```powershell
git fetch origin
git rev-list --count HEAD..origin/main
```
اگر عدد غیرصفر بود: توقف کامل، اول بیس را تازه کن (ریبیس روی `origin/main` طبق §۵)، بعد از اول شروع کن. هیچ داوری، شمارش چک یا تصمیم مرج روی بیس کهنه معتبر نیست — همان کلاسی که گیت `built_at_commit` برای گراف حل کرد، فقط برای خود کد.
...
## ۶. بعد از مرج همگام بمان
- `git fetch origin` را همیشه بزن (فقط رفرنس‌ها را به‌روز می‌کند، بی‌خطر است).
- داک/پلن/تیکت را همگام کن (ردیف TICKETS.md و STATE پلن).
- `pull` روی primary فقط وقتی که تمیز است؛ اگر کثیف است read-only می‌ماند و دست نمی‌خورد.
- claim را اگر گرفته‌ای آزاد کن؛ ورک‌تری و برنچ لوکال را پاک کن (برنچ ریموت با UI مالک).
اتمام: fetch ثبت شد، داک همگام است، primary یا pull شده یا آگاهانه دست‌نخورده مانده، و claim بازی نداری.
