...
|---|---|---|---|
| 1 | Persistence | services/db/__init__.py + services/db/schema.py + services/db/session_reports.py + services/db/query_results.py + services/db/legacy_aux.py + services/db/backup.py | get_conn() |
| 2 | AI/LLM Provider | services/ai/ai.py + services/ai/card_validation.py + services/ai/telemetry.py + services/ai/llm_services.py + services/ai/fallback_router.py + services/ai/ai_read_cache.py + services/ai/cache.py + services/ai/cost_resolver.py + services/ai/generation.py | _client(), ask_json(), ask_card(), ask_batch() |
| 3 | Session Assembly | services/session/__init__.py + assembly.py + summary.py + store.py + tier_registry.py | build_session_list(), generate_tier3_node(), build_report(), serialize_report(), deserialize_report(); store.py: SessionState, state_to_json(), state_from_json(), save_session(), load_session() (study_handler keeps thin delegates); tier_registry.py: iter_tier_candidates(), make_tier_node(), build_tier12_nodes(), next_tier_node() (assembly + srs_handler refill are thin callers) |
...
| 12 | Telegram UI -> AI Config | handlers/admin_ai.py (facade) + handlers/admin_ai_wizard.py + handlers/admin_ai_list.py + handlers/admin_ai_create.py + handlers/admin_ai_fallback.py + handlers/admin_ai_custom.py | handle_ai_callback, _show_ai_presets, _show_ai_settings |
