...
| 17 | Custom-word query orchestration | services/word_query.py (core) + bot.py ask-word block + handlers/srs_handler.py `_handle_query_add` + config/keyboards.py `query_result_keyboard` | ask(), toggle_save(); callback `query:add:` (the old `query:prepare:`/prepare() path was removed in #340 R3) |
