...
## Scope
- Move 3 probe functions (`zen_probe`, `google_probe`, `probe_pool`) into `factory/precard/net.py` + extract inline choose/retire/never-overwrite-empty logic; supervisor CLI calls them (no logic rewrite); `egress_pool.json` single writer; per-provider cooldown (429 on one provider never cools another).
