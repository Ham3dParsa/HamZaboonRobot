...
|---|---|---|---|---|
| 6.1 ✅ | `handlers/admin.py`: add `is_admin_awaiting()` + `handle_flow_back()` | medium | 7.8 (or alongside) | callback + module |
| 6.2 ✅ | `bot.py`: replace hard-coded prefix list at :394 with `is_admin_awaiting()` | low | 6.1 | callback |
| 6.3 ✅ | `bot.py`: move `flow:back` resume logic into `handle_flow_back()` | medium | 6.2 | callback |
...
| 6.1 | `handlers/admin.py`: add `is_admin_awaiting()` + `handle_flow_back()` | medium | 7.8 (or alongside) | callback + module |
| 6.2 | `bot.py`: replace hard-coded prefix list at :394 with `is_admin_awaiting()` | low | 6.1 | callback |
| 6.3 | `bot.py`: move `flow:back` resume logic (:445-463) into `handle_flow_back()` | medium | 6.2 | callback |
| 6.4 | Update/extend `tests/test_admin_awaiting.py` + wiring test | medium | 6.3 | wiring |

## Scope
- `handlers/admin.py`: add `is_admin_awaiting(awaiting) -> bool` and `handle_flow_back` (implemented as the flow-back/awaiting-resume function).
- `bot.py`: replace hard-coded prefix list at :403 with `is_admin_awaiting()`; move `flow:back` resume logic (:456-484) into `handle_flow_back()`.
- Owner gate at bot.py:254 unchanged (still rejects admin/llm_ awaiting for non-owner).
...
|---|---|---|
| Callback prefixes | `flow:back` (callback_router dispatch body) | kept; body moved into `handle_flow_back()` (admin.py), branch label unchanged |
| Router branches | `callback_router` `flow:back` block (was inline) → `handle_flow_back` call | moved; `if data == "flow:back":` branch itself unchanged |
| Keyboard builders / constants | none changed | keep |
| DB tables / columns / functions | none | keep |
| Handler functions | NEW `is_admin_awaiting()`, NEW `handle_flow_back()` in `handlers/admin.py` | added |
| Imports / re-exports | `bot.py` gains `is_admin_awaiting`, `handle_flow_back` from `handlers.admin`; drops `_edit_ai_preset` (now unused in bot.py) | updated; `handlers.admin._edit_ai_preset` still exported + used by `handle_flow_back` |
