...
STATE: phase 10/10 — status: in-progress — focus: T8-last DONE, ALL T8 sub-tickets complete (seam-12 migration done); Newline/HTML fix 694b3ae; group-manager gluing fix owner-approved; Kilo comments 3799335711/5716/410496 addressed + reviewer R3 gap + Kilo 3799467726/3799467729 escaping tests added (chain + usage). Remaining: Kilo loop, merge.
...
- T8-last DONE — `_confirm_save_preset` / `_delete_ai_preset` → spans (the only two T8-last HTML screens; `_activate_ai_preset`/`_confirm_delete_yes`/`_no`/`_duplicate_ai_preset`/`_discard_all_preset_changes`/`_save_ai_preset`/`_detach_ai_preset_group` are notify_callback toasts, out of R2 scope). Byte-exact verified (save/del confirm MATCH). Removed now-dead `html_escape` + `ParseMode` imports (verified zero remaining references; `_edit_or_send` plain-text calls remain, out of scope). New integration tests `test_confirm_screens_escape_preset_name_in_bold`, `test_fallback_chain_escapes_preset_name_in_bold`, `test_usage_page_escapes_preset_name` (Kilo 7726/7729). **No HTML `_edit_or_send` screens remain in admin_ai.py — seam-12 migration complete.** Full suite 1150 passed. (uncommitted)
