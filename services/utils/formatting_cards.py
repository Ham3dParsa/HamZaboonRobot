"""Card/SRS/session-summary leaf of the formatting seam (REF2-T2).

Verbatim home of the card renderings split out of
``services/utils/formatting.py``: the SRS staged-reveal prompt engine, the
word-query card builders, the saved-word fallback, phonetic lines, the
grammar-tip message, and the post-session summary/detail/legend renderings.
...
(one-directional cards→jalali; jalali never imports back, so no cycle).
``services.send_pretty`` stays a function-local import (never top-level —
that would invert the live ``helpers↔send_pretty`` edge).
"""
