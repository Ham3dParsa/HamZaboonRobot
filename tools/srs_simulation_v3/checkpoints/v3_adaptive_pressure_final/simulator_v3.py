...
class DailyRow:
    ...
    empty_slots: int
    total_cards: int
    pressure_level: float = 0.0  # 0.0 = relaxed, 1.0 = overloaded
...
def _pressure_query_suppression(pressure: float) -> float:
    """Pressure 0.0 -> 1.0 (full queries), Pressure 1.0 -> 0.3 (70% fewer queries)"""
    return max(0.3, 1.0 - pressure * 0.7)


def _pressure_save_suppression(pressure: float) -> float:
    """Pressure 0.0 -> 1.0 (full save rate), Pressure 1.0 -> 0.4 (60% fewer saves)"""
    return max(0.4, 1.0 - pressure * 0.6)
...
def simulate(cfg: SimConfig):
    ...
    for day in range(cfg.days):
        ...
        # Adaptive pressure: suppress queries & saves, boost LATER intervals
        if cfg.adaptive_pressure:
            pressure = min(2.0, len(active_cards) / max_sustainable)  # can exceed 1.0
            query_suppress = _pressure_query_suppression(pressure)
            save_suppress = _pressure_save_suppression(pressure)
            day_query_cap = max(1, int(day_query_cap * query_suppress))
            day_save_rate = day_save_rate * save_suppress
            day_forget = min(0.5, day_forget * (1.0 + pressure * 0.3))  # slight forget increase under pressure
        else:
            pressure = 0.0
        ...
        if cfg.adaptive_pressure and pressure > 0.7:
            ai_budget = int(ai_budget * (1.0 - pressure * 0.5))
        ...
        pressure = min(1.0, len(active_cards) / max_sustainable)
