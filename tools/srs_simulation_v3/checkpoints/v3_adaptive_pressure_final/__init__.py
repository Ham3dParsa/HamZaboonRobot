...
ARCHETYPE_CHOICES = ["lazy", "average", "motivated", "fluctuating"]
...
Examples:
  ...
  python -m tools.srs_simulation_v3 --user-archetype motivated --adaptive-pressure
  python -m tools.srs_simulation_v3 --user-archetype lazy --days 180 --csv lazy_run.csv
        """,
    )
    ...
    p.add_argument("--no-adaptive-pressure", action="store_true",
                    help="Disable adaptive load management")
    
    # User archetype
    p.add_argument("--user-archetype", choices=ARCHETYPE_CHOICES, default="average",
                    help="User behavior profile: lazy, average, motivated, fluctuating")
...
def _interactive_run() -> None:
    ...
    print("          randomized interval sets, user archetypes, adaptive pressure.")
    print("  (Disable with --no-three-buttons, --no-score, --no-random-intervals, --no-adaptive-pressure)")
    print()
    ...
    print("--- USER ARCHETYPE ---")
    print("lazy        - fewer queries, saves less, forgets more, skips sessions")
    print("average     - balanced baseline")
    print("motivated   - more queries, saves more, forgets less, consistent")
    print("fluctuating - 2-week motivation cycles (motivated <-> average)")
    archetype = _prompt("Archetype", default="average", choices=ARCHETYPE_CHOICES)
    print()

    print("--- DURATION ---")
    print("How many days to simulate?")
    print("  30 = one month, 60-90 shows long-term trends, 180+ = steady state")
...
def _build_config(args: argparse.Namespace) -> SimConfig:
    return SimConfig(
        ...
        adaptive_pressure=not args.no_adaptive_pressure,
