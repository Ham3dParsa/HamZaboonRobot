...
v4.2-sweet CHANGE (beyond v4.1):
  A. Per-card ease factor replaces fixed ladder intervals:
       Each card carries an ease multiplier [EASE_MIN, EASE_MAX] that adapts
        to the learner's self-reported recall quality: advance->+0.05,
        hold->-0.15, forget->max(EASE_MIN, ease - 0.3).  The actual interval becomes
       INTERVALS[idx] * ease * jitter, so a well-remembered card schedules
       *farther* out and a hard card comes back *sooner* — exactly matching
       real SRS behavior (SM-2's EF, minus per-user parameter fitting).
       A user-wide avg_ease is tracked and used as the starting ease for
       brand-new cards, so a learner who habitually rates "hard" gets
       shorter intervals from the start.

  B. Hold now advances the interval (not fixed as "pass"):
       Instead of leaving next_review unchanged (which kept the card due
       every single day), a hold outcome schedules the next review at
       half the current interval (INTERVALS[idx] * ease * 0.5).  This
       mirrors real behavior: "kinda remembered → come back sooner."
     ...
     model in bug 3). Two changes:
       - Score gains shrink as score approaches 5.0: gain *= (1 - score/5).
         ...
         every card from saturating near 5.0 after a handful of reviews,
            so "score >= 3.75" (v4.2-sweet) becomes a reachable secondary path.
...
GLOSSARY OF OUTPUT FIELDS:
  ...
  created_by_ai            - cards created via AI generation (accepted only)
    total_ai_calls           - AI calls from all sources (AI generation + user
                             queries); every attempt (gen attempt or query)
                             costs AI_COST_PER_CALL
    total_query_calls        - query-initiated AI calls (subset of total_ai_calls)
  ...
  final_active_cards       - cards in the active review pool at run end
    learned_words            - cards with idx>=3 OR score>=3.75 (primary KPI;
                             tightened from idx>=2/score>=4.0 in earlier versions;
  ...
  avg_user_ease            - mean ease factor across all reviewed cards;
                             captures how the user's self-rated recall quality
                             compresses (ease<1.0) or stretches (ease>1.0)
                             intervals relative to the fixed INTERVALS ladder
  avg_ease (daily row)     - same metric, reported per day
"""
...
# ---------------------------------------------------------------------------
# v4.2-sweet: Ease factor constants — per-card interval multiplier that adapts
# to the learner's self-reported recall quality.
#   advance: ease += EASE_ADVANCE_BOOST  (capped at EASE_MAX)
#   hold:    ease -= EASE_HOLD_DECAY     (floored at EASE_MIN)
#   forget:  ease = max(EASE_MIN, ease - EASE_FORGET_PENALTY) — subtract
#            0.3 to penalise without wiping historical ease entirely.
# ---------------------------------------------------------------------------
EASE_INIT = 1.0         # default ease for a brand-new card
EASE_MIN = 0.7          # floor — no card can schedule slower than 0.7x ladder
EASE_MAX = 1.5          # ceiling — no card can schedule faster than 1.5x ladder
EASE_ADVANCE_BOOST = 0.05    # ease gained on "remembers well"
EASE_HOLD_DECAY = 0.15       # ease lost on "was hard / borderline"
EASE_FORGET_PENALTY = 0.3    # ease subtracted on "forgot entirely" (relative decrement)
...
class Card:
    ...
    Attributes:
        ...
        ease: Per-card interval multiplier.  Starts at user_avg_ease (or
            EASE_INIT), then adapts: advance→ +0.05, hold→ -0.15,
            forget→ max(EASE_MIN, ease - EASE_FORGET_PENALTY).  Clamped to [EASE_MIN, EASE_MAX].
...
def _review_outcome(card: Card, persona: dict, cfg: SimConfig, rng: random.Random) -> str:
    ...
    Continuous mastery model (bug 3 fix, + diminishing-returns refinement):
        ...
        which is required for the "idx>=3 OR score>=3.75" learned-word
    ...
    v4.2-sweet — ease factor and hold fix:
        Ease adaptation no longer lives inside this function.  The caller
        (Step 7 in simulate()) reads the outcome and adjusts card.ease:
          advance: +0.05  (capped at EASE_MAX)
          hold:    -0.15  (floored at EASE_MIN)
          forget:  max(EASE_MIN, ease - EASE_FORGET_PENALTY)  (decrement 0.3)
        The ease multiplier directly scales the next interval:
          interval = INTERVALS[card.idx] * card.ease * jitter
        A "hold" now schedules at half that value so the card comes back
        sooner (instead of staying due every single day, which was the
        pre-v4.2-sweet behavior).
    ...
    if performance >= MASTERY_SUCCESS_THRESHOLD:
        card.idx = min(card.idx + 1, len(INTERVALS) - 1)
        card.streak += 1
        gain = 0.8 * (1.0 - card.score / 7.0)
        card.score = min(5.0, card.score + gain)
        return "advance"
    elif performance >= MASTERY_PARTIAL_THRESHOLD:
        gain = 0.15 * (1.0 - card.score / 7.0)
        card.score = max(0.0, min(5.0, card.score + gain))
        return "hold"
    else:
        card.idx = 0
        card.streak = 0
...
def simulate(cfg: SimConfig) -> tuple[list[dict], dict]:
    ...
    created_by_query = 0
    created_by_ai = 0
    total_query_calls = 0    # all query-initiated AI calls (saved + unsaved)
    lateness_samples = []
    ...
    for day in range(cfg.days):
        ...
        if attended:
            ...
            # Every user query costs an AI call (same as AI-generated cards).
            total_ai_calls += queries_today
            total_ai_cost += queries_today * AI_COST_PER_CALL
            total_query_calls += queries_today
        ...
        if attended:
            ...
            for c in candidates:
                ...
                if c.idx == -1:
                    c.idx = 0
                    if total_ease_count > 0:
                        c.ease = total_ease_sum / total_ease_count
                    ...
                    # --- ease factor update (v4.2-sweet) ---
                    if outcome == "advance":
                        c.ease = min(EASE_MAX, c.ease + EASE_ADVANCE_BOOST)
                    elif outcome == "hold":
                        c.ease = max(EASE_MIN, c.ease - EASE_HOLD_DECAY)
                    else:  # forget
                        c.ease = max(EASE_MIN, c.ease - EASE_FORGET_PENALTY)
                    total_ease_sum += c.ease
                    total_ease_count += 1
                    # --- interval computation with ease ---
                    jitter = 1.0 + rng.uniform(-cfg.jitter, cfg.jitter)
                    if outcome == "hold":
                        raw = max(1, round(INTERVALS[c.idx] * c.ease * 0.5))
                    else:
                        raw = max(1, round(INTERVALS[c.idx] * c.ease * jitter))
                    c.next_review = day + raw
                    c.due_since = None
            ...
            if cfg.enable_catchup and due_remaining > queue_cap_due * 0.25:
                ...
                for i in range(bonus_take):
                    ...
                    lateness_samples.append(day - c.due_since)
                    outcome = _review_outcome(c, persona, cfg, rng)
                    # ease update (mirrors Step 7 logic)
                    if outcome == "advance":
                        c.ease = min(EASE_MAX, c.ease + EASE_ADVANCE_BOOST)
                    elif outcome == "hold":
                        c.ease = max(EASE_MIN, c.ease - EASE_HOLD_DECAY)
        ...
        daily_avg_ease = round(total_ease_sum / total_ease_count, 2) if total_ease_count > 0 else EASE_INIT
        rows.append({
            ...
            "ai_cost_today": round((ai_generated + queries_today) * AI_COST_PER_CALL, 4),
            "bonus_processed": bonus_processed,
            "active": len(active),
            "avg_ease": daily_avg_ease,
        })

    learned = sum(1 for c in active if c.idx >= 3 or c.score >= 3.75)
    ...
    summary = {
        ...
        "avg_user_ease": round(total_ease_sum / total_ease_count, 2) if total_ease_count > 0 else EASE_INIT,
    }
    return rows, summary
...
def format_table(daily_log: list, summary: dict, cfg: SimConfig) -> str:
    ...
    ai_gen_only = summary['total_ai_calls'] - summary['total_query_calls']
    lines.append(f"  AI Calls (cost proxy):   {summary['total_ai_calls']} total "
                 f"(gen={ai_gen_only}, query={summary['total_query_calls']}, "
                 f"avg={summary['avg_ai_gen_per_day']}/day)")
    lines.append(f"  AI Rejected:             {summary['total_rejected_ai']}")
    lines.append(f"  AI Cost (USD):           ${summary['total_ai_cost_usd']}")
    lines.append(f"  Learned / $:             {summary['learned_words_per_dollar']} words/$")
    ...
    lines.append(f"  User Avg Ease:           {summary['avg_user_ease']} ({EASE_MIN}-{EASE_MAX})")
...
def write_csv(daily_log: list, path: str):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["day", "attended", "due_processed", "due_remaining",
                     ...
                     "bonus_processed", "active", "avg_ease"])
        for r in daily_log:
            w.writerow([r["day"], int(r["attended"]), r["due_processed"],
                        ...
                        r["bonus_processed"], r["active"], r["avg_ease"]])
