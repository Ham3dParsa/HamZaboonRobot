...
Usage: python3 test_simulator_v4.py
"""

import copy
import random
from simulator_v4 import SimConfig, PLAN_DEFAULTS, PERSONAS, PROFICIENCY, simulate, _resolve_plan
...
def section_toggles():
    ...
    log("| config | learned | avg_score | max_due | AI$ | learned/$ |")
    log("|---|---|---|---|---|---|")
    variants = {
    ...
    for name, kwargs in variants.items():
        ...
        s = run(cfg)
        log(f"| {name} | {s['learned_words']} | {s['avg_score']} | {s['max_due_backlog']} | "
...
def section_learning_curve():
    ...
    log("| day checkpoint | active_words | learned | avg_score | AI$ cumulative |")
    log("|---|---|---|---|---|")
    for days in [30, 90, 180, 360, 720]:
        ...
        log(f"| {days} | {s['total_active_words']} | {s['learned_words']} | "
            f"{s['avg_score']} | {s['total_ai_cost_usd']} |")
    log("")
...
def section_v3_vs_v4():
    ...
    log("| persona | mode | learned | avg_score | max_due | AI calls | AI$ |")
    log("|---|---|---|---|---|---|---|")
    for persona in PERSONAS:
        ...
        s_v4 = run(cfg_v4)
        log(f"| {persona} | v3-legacy | {s_legacy['learned_words']} | {s_legacy['avg_score']} | "
            f"{s_legacy['max_due_backlog']} | {s_legacy['total_ai_calls']} | {s_legacy['total_ai_cost_usd']} |")
        log(f"| {persona} | v4-full   | {s_v4['learned_words']} | {s_v4['avg_score']} | "
