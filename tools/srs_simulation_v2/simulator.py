...
Realistic user behavior:
  - Queries are randomized: user makes 0..query_daily_cap queries each day
  - Each query independently rolls for save chance
  - Total AI calls = queries_made + ai_generated (true API financial pressure)
...
def simulate(cfg: SimConfig):
    ...
    for day in range(cfg.days):
        # Step 1: User queries — randomized count + independent save rolls
        ...
        daily_ai_calls = queries_today + ai_used_today
        ...
        daily_rows.append(DailyRow(
            ...
            query_saved=len(query_backlog),
            ai_generated=ai_used_today,
            ai_calls=daily_ai_calls,
            ai_cap=cfg.ai_daily_cap,
            active_cards=len(active_cards),
        ))

    # ── Summary ──
    total_ai_gen = sum(r.ai_generated for r in daily_rows)
    total_ai_calls = sum(r.ai_calls for r in daily_rows)
    avg_ai = total_ai_gen / max(len(daily_rows), 1)
    avg_calls = total_ai_calls / max(len(daily_rows), 1)
    ...
    max_q_saved = max(r.query_saved for r in daily_rows)
...
def format_table(daily_rows: list[DailyRow], summary: Summary, cfg: SimConfig) -> str:
    ...
    header = (f"{'Day':>4} {'DueProc':>8} {'DueRem':>8} {'QSaved':>8} "
              f"{'AIGen':>6} {'AICalls':>8} {'AICap':>6} {'Active':>7}")
    sep = "-" * len(header)
    lines.append(header)
    lines.append(sep)
    ...
    for row in daily_rows:
        if cfg.report_every > 0 and row.day % cfg.report_every == 0:
            lines.append(
                ...
                f"{row.query_saved:>8} {row.ai_generated:>6} "
                f"{row.ai_calls:>8} {row.ai_cap:>6} {row.active_cards:>7}"
            )
    ...
    lines.append(
        ...
        f"{last.query_saved:>8} {last.ai_generated:>6} "
        f"{last.ai_calls:>8} {last.ai_cap:>6} {last.active_cards:>7}"
    )
    lines.append("")
    ...
    lines.append(f"  Avg Total AI Calls per day:       {summary.avg_ai_calls_per_day} "
                 f"(queries + AI gen = true API financial pressure)")
    lines.append(f"  Max Due Remaining (any day):      {summary.max_due_remaining}")
    lines.append(f"  Max Query Backlog (any day):      {summary.max_query_saved}")
...
def write_csv(daily_rows: list[DailyRow], summary: Summary, path: str) -> None:
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "day", "due_processed", "due_remaining", "queries_made",
            "query_saved", "ai_generated", "ai_calls", "ai_cap", "active_cards",
        ])
        for row in daily_rows:
            writer.writerow([
                row.day, row.due_processed, row.due_remaining,
                row.queries_made, row.query_saved, row.ai_generated,
                row.ai_calls, row.ai_cap, row.active_cards,
            ])
        writer.writerow([])
        writer.writerow(["summary", "value"])
        writer.writerow(["avg_ai_per_day", summary.avg_ai_per_day])
        writer.writerow(["avg_ai_calls_per_day", summary.avg_ai_calls_per_day])
        ...
        writer.writerow(["max_query_saved", summary.max_query_saved])
