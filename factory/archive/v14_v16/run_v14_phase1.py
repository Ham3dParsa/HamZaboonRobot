...
# keyword topics (LLM refine = phase 2, needs keys). Run: python -m factory.archive.v14_v16.run_v14_phase1
...
def _parse_args(argv=None):
    import argparse
    ap = argparse.ArgumentParser(description="v14 phase 1 (deterministic)")
    ap.add_argument("--pack", default=None,
                    help="language pack dir (default: <script>/packs/en)")
    ap.add_argument("--raw", default=None,
                    help="raw Kaikki jsonl (default: W:-only filtered_500 file)")
    ap.add_argument("--out", default=None,
                    help="output fixtures dir (default: <script>/fixtures)")
...
def _need(path, what):
    if not path.exists():
        raise SystemExit(
            "missing %s: %s\n(hint: %s)" % (what, path, _need.hint))
_need.hint = ("raw Kaikki + Tatoeba pool live on W: (see factory/README); "
              "pack files live in factory/packs/en (committed)")
...
def main(argv=None):
    ...
    PACK = pathlib.Path(args.pack) if args.pack else PACK
    OUT = pathlib.Path(args.out) if args.out else OUT
    RAW = pathlib.Path(args.raw) if args.raw else RAW
    OUT.mkdir(parents=True, exist_ok=True)
    _need(PACK / "pack.json", "pack manifest")
    _need(PACK / "lemmas.csv", "lemma list")
    _need(RAW, "raw Kaikki jsonl")
