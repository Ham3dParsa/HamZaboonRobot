...
- `services/activity_log.py`: single owner of the `USER_ACTIVITY` diagnostic log (`log_user_activity`); consolidates the former logging blocks in `handlers/user.py` and `handlers/srs_handler.py`.
  ...
  - `services/utils/`: `callback_notifications.py`, `formatting_escape.py` (escape/digits/HTML leaves), `formatting_jalali.py` (jalali/date labels + `parse_iso_to_app_tz`), `formatting_cards.py` (card/SRS/session-summary renderings), `formatting.py` (facade re-export shim), `helpers_pure.py` (pure log-level/activity/normalize leaves), `helpers_awaiting.py` (awaiting-prompt tracking), `helpers_llm.py` (LLM wait-state + awaiting exit + `say` adapter), `helpers_retry.py` (Telegram retry loops), `helpers.py` (facade re-export shim), `validation.py`.
