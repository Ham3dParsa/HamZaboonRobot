...
- `handlers/`: Telegram handlers.
  - `handlers/admin.py`: thin admin dispatcher; registers admin/llm routes in `services/routing`; admin awaiting flows in `handlers/flows.py`; admin-infra handlers (broadcast, settings) — backup/restore delegated to `handlers/admin_backup.py`.
  ...
  - `handlers/admin_stats.py`, `admin_plans.py`, `admin_cost.py`, `admin_ai.py`: admin sub-routers (stats / plans / cost / AI presets). `admin_ai.py` is a thin facade over `admin_ai_wizard.py`/`admin_ai_list.py`/`admin_ai_create.py`/`admin_ai_fallback.py`/`admin_ai_custom.py` leaves.
...
- `services/`: domains.
  ...
  - `services/quota_service.py`: thin kind-dispatched quota facade (`can`/`reserve`/`consume`/`release` over `word`/`grammar_tip`/`session_slot` + re-exported owner bodies); SQL stays in `services/db/users.py` and `services/scheduling.py`.
  - `services/grade_service.py`: thin activity-dispatched grade facade (`grade()` over `srs_review`/`first_exposure` + re-exported owner bodies); formulas + single `transaction()` stay in `services/db/words.py`.
- `services/activity_log.py`: single owner of the `USER_ACTIVITY` diagnostic log (`log_user_activity`); consolidates the former logging blocks in `handlers/user.py` and `handlers/srs_handler.py`.
  - `services/db/`: SQLite schema/migrations/persistence/quota. `__init__.py` thin re-export façade; `schema.py` owns schema/migrations; `plans.py` seeds+CRUDs plans; `settings.py` settings accessors; `cost_tracking.py` LLM cost; `preset_registry.py` AI preset/fallback/hourly-usage; `display_toggles.py` display-toggle state + precedence; `key_crypto.py` key encryption (fail-closed); `session_reports.py` persisted post-session report store (R10); `query_results.py` cached query-result cache + cap/dedup; `legacy_aux.py` retired grammar-tips + config-tests audit leaves; `backup.py` snapshot export + validated restore.
  - `services/ai/`: OpenAI-compatible client, JSON extraction, validation, prompts, generation, presets. `card_validation.py` owns pure card validation (`validate_card`, `card_repair_fields`, `validate_card_patch`, `validate_batch`; `ai.py` re-exports aliases). `telemetry.py` owns tracked-request telemetry (`_log_llm_request`, `_call_tracked`, `TrackedResult`; `ai.py` re-exports aliases). `preset_fields.py` owns canonical AI-preset field schema. `cache.py` owns the generic thread-safe `TTLCache`; `ai_read_cache.py` is the thin chain/cost-profile facade over it for BOT-1/2 amplification reduction. `cost_resolver.py` owns preset-dict-first cost resolution (`resolve_costs`) + the secret-free client pool-key helper (`client_pool_key`); name/group-key caching is rejected-deferred (no such cache). `fallback_router.py` owns chain routing (`_call_ai_limited` + rate/daily checks + `_retry_primary_preset` + `AllPresetsExhausted`/`AIRequestTimedOut`); `generation.py` owns card generation (`_get_active_preset` + `_ask_batch_limited` + `_prepare_cached_card`); `llm_services.py` is a pure re-export facade (zero defs).
  - `services/utils/`: `callback_notifications.py`, `formatting_escape.py` (escape/digits/HTML leaves), `formatting_jalali.py` (jalali/date labels + `parse_iso_to_app_tz`), `formatting_cards.py` (card/SRS/session-summary renderings), `formatting.py` (facade re-export shim), `helpers.py` (retry/cancel), `validation.py`.
...
- `factory/precard/prune.py`: S1 sense pruner (R2 hard drops → R4 niche guard → R3 twin dedup); single owner of the S1 prune chain.
